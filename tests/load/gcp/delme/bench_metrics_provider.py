...
prom_scrape_avalanche_up = Gauge("prom_scrape_avalanche_up", "Prom scraped avalanche successfully")

juju_up = Gauge("juju_up", "Juju is responsive")
...
def get_scrape_duration():
    ...
    targets_info = get_json_from_url(
        targets_url
    )  # json.loads(get_stdout(["curl", "--no-progress-meter", targets_url]))
    ours = list(
        filter(
            lambda target: target["discoveredLabels"]["__address__"] == "192.168.1.101:9001",
            targets_info["data"]["activeTargets"],
        )
    )
...
def get_scrape_interval() -> int:
    ...
    config_info = get_json_from_url(
        config_url
    )  # json.loads(get_stdout(["curl", "--no-progress-meter", config_url]))
    config_info = yaml.safe_load(config_info["data"]["yaml"])
    ours = list(
        filter(
            lambda scrape_config: set(scrape_config["static_configs"][0]["targets"])
            >= {"192.168.1.101:9001"},
